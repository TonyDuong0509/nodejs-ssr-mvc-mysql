...
class Base {
  ...
  destroy = async () => {
    try {
      await pool.execute(`DELETE FROM ${this.TABLE_NAME} WHERE id=?`, [
        this.id,
      ]);
      return true;
    } catch (error) {
  ...
  save = async (data) => {
    ...
    try {
      const sql = `INSERT INTO \`${this.TABLE_NAME}\` (${colNames.join(
        ","
  ...
  update = async () => {
    try {
      ...
      values.push(this.id);
      await pool.execute(
        `UPDATE ${this.TABLE_NAME} SET  ${cols.join(",")} WHERE id=?`,
        values
      );
      return true;
    } catch (error) {
